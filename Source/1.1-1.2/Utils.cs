...
namespace aRandomKiwi.MFM
{
    static class Utils
    {
        public static string releaseInfo = "Update 1.5.22";
        public static string releaseDesc = "-Fixed a critical bug that led to the loss of some or all of the rented mercenaries.\n-Fixed a bug which can broke the USFM rented mercs 'recall' dialog.\n-Fixed the ability to recall already recalled rented mercs\n-Fixed issue allowing 'recalled' rented mercenaries to be listed and paid in USFM bills.";
                    ...
                    for (int i = 0; i < allParts.Count; i++)
                    {
                        ...
                        switch (bionicSource) {
                            ...
                            case 0:
                                ...
                                else if (bp.def == BodyPartDefOf.Stomach && !Utils.MSELOADED)
        ...
        public static IntVec3 spawnMercOnMap(Map map, List<Pawn> toDeliver)
        {
            ...
            else
            {
                ...
                IntVec3 root;
                if (CellFinder.TryFindRandomEdgeCellWith((IntVec3 x) => baseValidator(x) && ((hostFaction != null && map.reachability.CanReachFactionBase(x, hostFaction)) || (hostFaction == null && map.reachability.CanReachBiggestMapEdgeRoom(x))), map, CellFinder.EdgeRoadChance_Neutral, out root))
                {
        ...
        public static bool spawnMedievalCaravan(Map map, List<Thing> thingsToSpawn, out IntVec3 spawnedThings)
        {
            ...
            {
                Log.Warning("Failed to do traveler incident from " + spawnCenter + ": Couldn't find anywhere for the traveler to go.", false);
                return false;
            }
        ...
        public static void payLocalSilver(int debt, Map map, ITrader trader)
        {
            ...
            IL_CC:
                if (thing == null)
                {
                    Log.Error("Could not find any " + ThingDefOf.Silver + " to transfer to trader.", false);
                    break;
                }
        ...
        public static void createSiteOfPayment(Map map, int type=-1)
        {
            ...
            int dstTile = 0;
            TileFinder.TryFindNewSiteTile(out dstTile, 2, 5, true, true, map.Tile);
            sop.Tile = dstTile;
            if(type != -1)
